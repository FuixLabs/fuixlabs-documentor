...
export const changeHoldership = async (currentUserPublicKey, args) => {
  ...
  try {
    ...
    if (!isSurrender) {
      ...
      dids = {
        ...dids,
        [transferPerson]: generateDid(
          COMPANY_NAME, requestResult?.data ? requestResult?.data?.publicKey : null
        ),
      };
      publicKey = confirmNominate ? keys[action.updatedFieds[0].name] : requestResult?.data ? requestResult?.data?.publicKey : null
