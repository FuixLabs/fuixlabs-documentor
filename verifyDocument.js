...
export const verifyWrappedDocument = async (
  ...
  if (service !== SERVICE.CARDANO) {
    ...
    try {
      const res = await verifyCardanoDocument(document, access_token);
 ...
 * @return {Promise}
 */
const verifyCNFT = async (targetHash, policyId, access_token) => {
  try {
    const response = await requestVerifyCNFT(
...
const verifySignature = async (
  ...
  try {
    const verifySigRes = await requestVerifySignature(
      ...
      {address, payload, signature, key},
      access_token,
    );
    if (!verifySigRes.data || verifySigRes.data.error_code) {
      throw verifySigRes.data.error_message;
    }
  } catch (e) {
...
export const verifyCardanoDocument = async (document, access_token) => {
  try {
    ...
    if (res && !res.error_code) {
      ...
      const signature = document?.signature?.proof[0]?.signature?.signature;
      const key = document?.signature?.proof[0]?.signature?.key;
      const didOfWrappedDocument = document?.data?.did;
      const policyId = document?.mintingNFTConfig?.policy?.id;
      ...
      await verifyCNFT(targetHash, policyId, access_token);
      await verifySignature(
        unsalt(document?.data.issuers[0]?.address),
        payload,
        signature,
        key,
        access_token,
